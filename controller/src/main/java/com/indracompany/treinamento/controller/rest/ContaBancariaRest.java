...
public class ContaBancariaRest extends GenericCrudRest<ContaBancaria, Long, ContaBancariaService>{
	...
	public @ResponseBody ResponseEntity<List<OperacaoConta>> consultarExtrato(@PathVariable String agencia,
		...
		List<OperacaoConta> extrato = contaBancariaService.consultarExtrato(agencia, conta);
		return new ResponseEntity<>(extrato, HttpStatus.OK);
	}
	
	@GetMapping(value = "/consultar-extrato-data/{agencia}/{conta}/{dataInicial}/{dataFinal}", produces = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody ResponseEntity<List<OperacaoConta>> consultarExtratoPorData(@PathVariable String agencia,
			@PathVariable String conta, @PathVariable String dataInicial, @PathVariable String dataFinal) {
		
		List<OperacaoConta> extrato = contaBancariaService.consultarExtratoPorData(agencia, conta, dataInicial, dataFinal);
